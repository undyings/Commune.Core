...
namespace Site.Engine
{
  public class Labels
  {
    public readonly static ThreadLabel Service = new("Service", ThreadPriority.Lowest);
		public readonly static ThreadLabel Mailing = new("Mailing", ThreadPriority.Lowest);
  }
  ...
  public class SiteTasks
  {
		...
		public static IEnumerable<Step> SitemapXmlChecker(IContext context, string sitemapDirectory)
		{
			...
			Log.Information("SitemapXmlChecker стартовал: {0}", sitemapPath);
			...
			while (!context.Pull.IsFinishing)
			{
				...
				Log.Information("sitemap.xml успешно обновлен");
			}
		}


		public static IEnumerable<Step> SitemapXmlChecker(IContext context,
			string sitemapDirectory, Func<LinkInfo[], IEnumerable<LightLink>> linksGetter)
		{
			string sitemapPath = Path.Combine(sitemapDirectory, "sitemap.xml");
    ...
    {
      Log.Information("CleaningSessions стартовал");
  ...
  //  }
		...
		//public static IEnumerable<Step> CatalogYmlChecker(IContext context, string catalogPath,
		//	Getter<YmlElement> catalogGetter)
		//{
		//	Logger.AddMessage("CatalogYmlChecker стартовал: {0}", catalogPath);

		//	yield return new WaitStep(TimeSpan.FromSeconds(30));
		...
		//		if (File.Exists(catalogPath) &&
		//			context.Store.CreateTime - File.GetLastWriteTimeUtc(catalogPath) < TimeSpan.FromHours(1))
		//		{
		//			FileInfo file = new FileInfo(catalogPath);
		//			if (file.Length > 10)
		//				continue;
		//		}

		//		YmlWriterHlp.ExportCatalog(catalogPath, catalogGetter());

		//		Logger.AddMessage("Yml каталог успешно обновлен");
		//	}
		//}
