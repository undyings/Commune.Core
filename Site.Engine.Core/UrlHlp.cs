...
namespace Site.Engine
{
  public class UrlHlp
  {
    ...
    static string imagesPath
    {
      get
      {
        return BaseContext.Default.ImagesPath;
      }
    }
  ...
  //  }
    ...
    {
      string filePath = Path.Combine(BaseContext.Default.WebRootPath, subPaths.Replace('/', '\\').TrimStart('\\'));
    ...
    {
			imageSubPaths = imageSubPaths.Where(sp => !StringHlp.IsEmpty(sp)).ToArray();
			FileInfo imageInfo = new(ImagePath(imageSubPaths));
      if (!imageInfo.Exists)
    ...
    public static string ShopUrl(string kind, int? id = null, int? parentId = null)
    {
      ...
      string directory = BaseContext.Default.Store.Links.ToDirectory(id.Value, parentId);
      if (directory != null)
    ...
    {
      return Path.Combine(BaseContext.Default.ImagesPath, objectId.ToString(), "gallery", imageName);
    }

    public static string GalleryThumbPath(int objectId, string imageName)
    {
      string thumbName = Path.ChangeExtension(imageName, "png");
      return Path.Combine(BaseContext.Default.ImagesPath, objectId.ToString(), "thumb", thumbName);
    }
