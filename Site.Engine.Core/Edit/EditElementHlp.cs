  ...
  public class EditElementHlp
  {
      ...
      Func<bool>? actionEnabler)
    {
      ...
      return GetActionPanel(httpContext, state, popupDialog, "Удалить", objectKind, dialogTitle, actionEnabler,
        delegate
        {
          ...
          DataBox.RemoveParentObjectWithChilds(dbContext, objectId);

          dbContext.SaveChanges();

          Log.Information("{0} '{1}' успешно удален", StringHlp.ToUpper(objectKind, 1), objectId);
						...
						.Event(string.Format("delete_{0}", imageName), "",
							delegate (JsonData json)
							{
								...
								if (File.Exists(imagePath))
								{
									...
									Log.Information("Файл '{0}' успешно удален", imagePath);
								}
							}
						)
				).PositionRelative()
   ...
   // }
    ...
    public static IHtmlControl? GetOperationPopup(WebOperation operation)
    {
      ...
      bool isWarning = operation.IsWarning;
