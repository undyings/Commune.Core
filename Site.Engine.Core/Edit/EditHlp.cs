...
namespace Site.Engine
{
  public class EditHlp
  {
			...
			int? parentId, int? unitId, string fixedDesignKind, out string title)
    {
      ...
      SectionStorage sections = BaseContext.Default.Store.Sections;
			...
			int? parentId, int? sectionId, string fixedDesignKind, out string title)
    {
      ...
      string designKind = section.Get(SectionType.DesignKind);
				...
				EditElementHlp.GetButtonsPanel(
					...
					.Event("save_article", "editContent",
						delegate (JsonData json)
						{
							...
							LightObject? editContacts = DataBox.LoadObject(fabricConnection, dbContext, ContactsType.Contacts, ContactsType.Kind.Create(new KindId("main")));
							...
							editContacts.Box.Update(dbContext, true);
		...
		public static IHtmlControl GetCatalogueEdit(EditState state, DisplayName[] catalogueSections, out string title)
		{
			title = "Справочник";
			...
			return new HPanel(
				DecorEdit.Title(title),
				new HPanel(
					catalogueSections.Select((section) =>
					{
						return new HPanel(
							DecorEdit.RedoButton(section.Display, UrlHlp.EditUrl(section.Name, null))
						).MarginBottom(5);
					}).ToArray()
