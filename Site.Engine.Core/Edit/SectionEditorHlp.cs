        ...
        EditElementHlp.GetButtonsPanel(
          DecorEdit.AddButton("Добавить").Event(initiator, "add_section", "addContent",
            delegate (JsonData json)
            {
              ...
              {
                state.Operation.Warning("Не задан заголовок раздела");
                return;
              }
              ...
              if (editParent == null)
              {
                state.Operation.Error("Не найден родительский раздел");
                return;
              }

              ObjectBox box = new(fabricConnection, dbContext);
              int? createSectionId = box.CreateUniqueObject(dbContext, SectionType.Section,
                SectionType.Title.Create(new NameId(sectionName)), null);

              if (createSectionId == null)
              {
                state.Operation.Warning("Раздел с таким заголовком уже существует");
                return;
              }
              ...
              LightObject addSection = new(box, createSectionId.Value);
              addSection.SetCreateTime();

              addSection.Set(SectionType.DesignKind, designKind);

              editParent.AddChildLink(SectionType.SubsectionLinks, addSection.Id);
              //addSection.AddParentId(SectionType.SubsectionLinks, parent.Id);

              addSection.Box.Update(dbContext, false);
      ...
      return new HPanel(
        ...
        EditElementHlp.GetDeletePanel(httpContext, initiator, state, section.Id, "раздел", "Удаление раздела",
          delegate
          {
            if (section is LightSection && ((LightSection)section).Subsections.Length > 0)
            {
              state.Operation.Warning("Раздел имеющий подразделы не может быть удален");
              return false;
            }
        ...
        EditElementHlp.GetButtonsPanel(
          ...
          .Event(initiator, "save_section", "editContent",
            delegate (JsonData json)
            {
              ...
              LightObject? editSection = DataBox.LoadObject(fabricConnection, dbContext, section.Id);
              ...
              if (!SectionType.Title.SetWithCheck(dbContext, editSection.Box, editSection.Head, new NameId(title)))
              {
                state.Operation.Warning("Страница с таким заголовком уже существует");
                return;
              }
              ...
              editSection.Box.Update(dbContext, true);
