...
namespace Site.Engine
{
  public static class DecorEdit
  {
    ...
    {
      return AdminPagePanel(state, section, "");
    }
    ...
    {
      return AdminUnitRedoBlock(editMode, unitId, "");
    }
    ...
    public static IHtmlControl? AdminUnitRedoBlock(bool editMode, int unitId, string fixedDesignKind)
    {
      ...
      return new HPanel(
        ...
        DecorEdit.AdminUnitPanel(fixedDesignKind != "", unitId, fixedDesignKind)?.MarginLeft(6)
    ...
    {
      return new HLink(url,
        new HButton("", std.BeforeAwesome(@"\f044", true, 0)).Color(iconColor)
    ...
    {
      HEventPanel wrapperPanel = new HEventPanel(popupPanel.Media(width, new HStyle().BorderRadius(0)))
        .Width(width).OnClickWithStopPropagation().Block() //.VAlign(null).Align(null)
        .PositionAbsolute().Left("50%").Top("50%").Transform("translateX(-50%) translateY(-50%)")
				.Media(width, new HStyle().WidthFull().Top(0).Transform("translateX(-50%)"));

			return new HPanel(
        wrapperPanel
			).Position("fixed").ZIndex(1000).Left(0).Top(0).Right(0).Bottom(0)
		...
		static IHtmlControl? GetDialogIcon(DialogIcon iconKind)
    {
      switch (iconKind)
      {
